...
const NonFlammablePropellantPage = () => {
  ...
  useEffect(() => {
    const loadContent = async () => {
      setLoading(true);
      try {
        const aboutContent = await fetchNonFlammablePropellantContent();
  ...
  const splitContentIntoParagraphs = (content: string): string[] => {
    // Split by the border-t div which separates the two sections
    const parts = content.split('<div class="border-t border-gray-200 dark:border-gray-700 pt-6">');
    
    if (parts.length >= 2) {
      const firstPart = parts[0].trim();
      const secondPart = '<div class="">' + parts[1].trim();
      return [firstPart, secondPart];
    }
    
    return [content, defaultParagraph2];
  };
      ...
      <main className="pt-20 pb-16 bg-gray-50 dark:bg-gray-900">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            {/* Static Benefits Section - Not Editable */}
